...
export async function generateMetadata({ params }: { params: Promise<{ country: string }> }): Promise<Metadata> {
  ...
  const data = countryData[decodedCountry];
  if (!data) return {};
  const cities = getCitiesBySlug(decodedCountry);
  return {
    title: `Today Prayer Times ${data.name} | Namaz Timings All Cities in ${data.name}`,
    ...
    alternates: { canonical: `https://noor.souqalmadina.com.pk/prayer-times/${decodedCountry}` },
  };
}
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-5 text-xs text-white/45 font-bold uppercase tracking-wider flex items-center gap-2">
        ...
        <span>/</span>
        <Link href="/prayer-times/countries" className="hover:text-white transition-colors">Countries</Link>
        <span>/</span>
      ...
      <main className="max-w-4xl mx-auto px-4 pb-14 w-full flex-1">
        <PaginatedGrid
          ...
          items={cities.map((city) => ({
            name: city,
            href: `/prayer-times/${decodedCountry}/${slugifyCityName(city)}`,
          }))}
          perPage={30}
        />
        ...
        <article className="mt-12 p-8 bg-white/90 rounded-3xl">
          ...
          <p className="text-gray-600 leading-relaxed">
            ...
            ensures you never miss Fajr, Dhuhr, Asr, Maghrib or Isha. Our data is calculated offline 
            using precise astronomical algorithms for maximum reliability.
          </p>
        </article>
      </main>
